'''
This file is for validating regressed electricity demand data from resstock and
comstock. "ref" is the direct EULP simulation total for a given weather year;
"reg" is the regression prediction for the same weather year, trained on a
different weather year's EULP data.

The aggregation pipeline writes per-enduse files now (cooling.elec and
heating.elec separately, no combined HVAC.elec). This script produces one
validation report per (ref/reg pair × enduse) — so each ref/reg pair yields
two reports, letting you see whether the cooling-only and heating-only models
have different cross-AMY-year accuracy profiles.

This script was tested with the buildstock-weatherreg project environment
(see pyproject.toml).
'''
import certifi
import os
...
TARGET_YEAR = 2018
# Per-enduse agg files the pipeline writes (one GWh CSV per enduse per run).
# Order is just for stable iteration / log readability — each enduse runs
# independently as its own validation report.
...
def build_comparisons_from_switches(switches_path: Path):
    ...
    (apply_regression=False) and reg specs (apply_regression=True), and
    further expands each pair into ONE COMPARISON PER ENDUSE (cooling_elec
    and heating_elec separately). The ref's `base_year` is the comparison
    year; reg files (which may carry multiple target years) are truncated
    to that year at load time.
    ...
    # upgrade so we never compare a baseline against a measure or vice versa.
    by_upgrade = {}
    ...
    for upgrade_id, specs in by_upgrade.items():
        ...
        for ref, reg in itertools.product(refs, regs):
            ...
            for enduse in ENDUSES:
                ref_path = output_dir / (
                    f"agg_{bldg_type}_eulp_{enduse}_GWh_upgrade{ref_tag}.csv"
                )
                reg_path = output_dir / (
                    f"agg_{bldg_type}_eulp_{enduse}_GWh_upgrade{reg_tag}.csv"
                )
                comparisons.append({
                    'name': f"upgrade {upgrade_id}: ref_b{ref_y} vs reg_b{reg_y} [{enduse}]",
                    'ref': ref_path,
                    'reg': reg_path,
                    'target_year': ref_y,
                    'slug': f"validation_ref{ref_y}_reg_b{reg_y}_{enduse}",
                })
...
def main():
    parser = argparse.ArgumentParser(
        description="Run validation comparisons from a run's switches JSON. "
                    "Each ref/reg pair in run_specs produces two reports "
                    "(one per enduse) under the run's output_dir."
    )
    parser.add_argument(
        '--switches', type=Path, required=True,
        help="Path to a run's switches JSON. Comparisons are built from "
             "run_specs (cross of refs × regs per upgrade_id × enduse). "
             "Output subfolders named "
             "'validation_ref<RY>_reg_b<BY>_<enduse>' land directly in "
    ...
    for i, comp in enumerate(comparisons, 1):
        ...
        if missing:
            ...
            continue
        # Catch SystemExit from inside run_comparison (e.g., year filter
        # leaves no rows when ref's base_year isn't in reg's target_years,
        # or zero common timestamps after alignment) so one bad pair
        # doesn't kill the whole batch. Logs the reason and moves on.
        try:
            run_comparison(
            ...
            log(f"  [skip] {comp_name}: {e}")
            # ensure_output_dir creates the subdir before load_csv fails;
            # clean up the empty skeleton so listings stay tidy.
            subdir = OUTPUT_DIR / comp['slug']
            try:
